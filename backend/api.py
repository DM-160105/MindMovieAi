...
# ═══════════════════════════════════════════════════════════════════════════════
# ─── VIBE — Cinematic Atmosphere Fingerprint Recommender ──────────────────
# ═══════════════════════════════════════════════════════════════════════════════

# Module-level state (loaded lazily on first request, like _arc_pca_model)
_vibe_keyword_map: Optional[dict] = None
_vibe_presets_cache: Optional[list] = None

# Import dimension_keywords (works both locally and on Render)
try:
    from dimension_keywords import DIMENSIONS as _VIBE_DIMENSIONS, VIBE_PRESETS as _VIBE_PRESETS_DEFAULT, DIMENSION_KEYWORDS as _DIM_KW_DEFAULT
except ImportError:
    _VIBE_DIMENSIONS = ["lighting", "pacing", "setting_type", "temperature", "texture", "era_feel"]
    _DIM_KW_DEFAULT = {}
    _VIBE_PRESETS_DEFAULT = []


def _load_vibe_config(db):
    """Load vibe keyword map + presets from MongoDB (once per startup)."""
    global _vibe_keyword_map, _vibe_presets_cache
    if _vibe_keyword_map is not None:
        return
    try:
        kw_doc = db["vibe_config"].find_one({"_id": "dimension_keywords"})
        if kw_doc:
            _vibe_keyword_map = kw_doc.get("keywords", {})
        else:
            _vibe_keyword_map = _DIM_KW_DEFAULT

        preset_doc = db["vibe_config"].find_one({"_id": "vibe_presets"})
        if preset_doc:
            _vibe_presets_cache = preset_doc.get("presets", [])
        else:
            _vibe_presets_cache = _VIBE_PRESETS_DEFAULT
    except Exception as e:
        print(f"⚠️  Vibe config load error: {e}")
        _vibe_keyword_map = _DIM_KW_DEFAULT
        _vibe_presets_cache = _VIBE_PRESETS_DEFAULT


def _tokenize(text: str) -> list[str]:
    """Basic tokenizer: lowercase, remove punctuation, strip NLTK stopwords."""
    import re
    try:
        import nltk
        try:
        ...
        sw = {"a", "an", "the", "and", "or", "but", "in", "on", "at", "to",
...
def _text_to_vibe_vector(text: str, keyword_map: dict) -> list[float]:
    """
    Convert free-text vibe description to a 6-dim float vector.
    Pure Python — no ML model. Uses keyword lookup + partial-match penalty.
    """
    tokens = _tokenize(text)
    scores: dict[str, list[float]] = {dim: [] for dim in _VIBE_DIMENSIONS}

    for token in tokens:
        # Also try bigrams/trigrams from the original text
        for dim, keywords in keyword_map.items():
            # Exact match
            if token in keywords:
                scores[dim].append(keywords[token])
            else:
                # Partial: token appears in a multi-word keyword
                for kw, score in keywords.items():
                    kw_parts = kw.lower().split()
                    if token in kw_parts:
                        scores[dim].append(score * 0.7)
                        break

    # Also try bigrams from original text
    ...
    for phrase in bigrams + trigrams:
        for dim, keywords in keyword_map.items():
            if phrase in keywords:
                scores[dim].append(keywords[phrase])

    return [sum(v) / len(v) if v else 0.5 for v in scores.values()]
...
class VibeRateRequest(BaseModel):
    movie_id: str
    vibe_match_felt: int   # 1–5
    comment: Optional[str] = None


# ── Endpoints ─────────────────────────────────────────────────────────────────

@app.get("/api/vibe-presets")
def get_vibe_presets(db=Depends(get_db)):
    """Return the 24 curated vibe preset objects with pre-computed 6-dim vectors."""
    _load_vibe_config(db)
    return {"presets": _vibe_presets_cache or _VIBE_PRESETS_DEFAULT}


@app.post("/api/vibe-match")
def vibe_match(req: VibeMatchRequest, db=Depends(get_db)):
    """
    Main Vibe recommendation endpoint.

    1. Convert vibe_text → 6-dim query vector (or use preset vector directly)
    2. MongoDB: fetch up to 200 candidates sorted by rating (quality pre-filter)
    3. NumPy cosine similarity on the 6×N matrix
    4. Return top `limit` with per-dimension closeness scores
    """
    _load_vibe_config(db)
    keyword_map = _vibe_keyword_map or _DIM_KW_DEFAULT
    limit = min(req.limit or 10, 20)

    # Build query vector
    query_vector: list[float] = [0.5] * 6
    query_tags: list[str] = []

    if req.preset_id and _vibe_presets_cache:
        preset = next((p for p in _vibe_presets_cache if p["id"] == req.preset_id), None)
        if preset:
            query_vector = preset["vector"]
            query_tags = preset.get("id", "").replace("-", " ").split()
    elif req.vibe_text and req.vibe_text.strip():
        query_vector = _text_to_vibe_vector(req.vibe_text, keyword_map)
        query_tags = _tokenize(req.vibe_text)[:8]
    else:
        return {
            "results": [],
            "query_vector": query_vector,
            "query_tags": [],
            "message": "Provide vibe_text or preset_id",
        }
    ...
    mongo_query: dict = {}
    if req.genres:
        mongo_query["genres"] = {"$in": req.genres}

    candidates = list(
        db["vibe_movies"].find(mongo_query).sort("rating", -1).limit(200)
    )
    ...
    if not candidates:
        return {
            ...
            "message": "No movies found. Run seed_vibes.py to populate the database.",
        }

    # Filter valid vectors + build matrix
    valid, vecs = [], []
    exclude_set = set(req.exclude_ids or [])
    for c in candidates:
        if str(c.get("_id")) in exclude_set:
            continue
        vec = c.get("vibe_vector")
    ...
    if not valid:
        return {
            ...
            "query_vector": query_vector,
            "query_tags": query_tags,
            "message": "No valid vibe vectors found. Run seed_vibes.py.",
        }

    matrix = np.array(vecs, dtype=np.float32)
    raw_scores = _cosine_similarity_batch(query_np, matrix)

    # Sort descending
    scored = sorted(zip(valid, raw_scores.tolist()), key=lambda x: x[1], reverse=True)
    top = scored[:limit]

    # Build response
    results = []
    for movie, score in top:
        movie_vec = movie.get("vibe_vector", [0.5] * 6)
        # Per-dimension closeness: 1 - abs(movie_dim - query_dim)
        dimension_match = {
            dim: round(1.0 - abs(movie_vec[i] - query_vector[i]), 3)
            for i, dim in enumerate(_VIBE_DIMENSIONS)
        }
        # Normalise match score to 0-1 (cosine is already -1 to 1)
        match_score = round(float(max(0.0, min(1.0, (score + 1) / 2))), 3)
        results.append({
            "id": str(movie.get("_id", "")),
            "title": movie.get("title", ""),
            "year": movie.get("year"),
            ...
            "rating": movie.get("rating", 0),
            "overview": movie.get("overview", ""),
            "vibe_match_score": match_score,
            "vibe_tags": movie.get("vibe_tags", []),
            "vibe_summary": movie.get("vibe_summary", ""),
            "vibe_vector": movie_vec,
...
@app.get("/api/vibe-movie/{movie_id}")
def get_vibe_movie(movie_id: str, db=Depends(get_db)):
    """Fetch a single movie with full vibe data by its MongoDB _id."""
    from bson import ObjectId

    try:
        doc = db["vibe_movies"].find_one({"_id": ObjectId(movie_id)})
    except Exception:
        doc = db["vibe_movies"].find_one({"tmdb_id": int(movie_id) if movie_id.isdigit() else 0})

    if not doc:
        raise HTTPException(status_code=404, detail="Movie not found in vibe_movies collection")
    ...
    return {
        ...
        "rating": doc.get("rating", 0),
        "overview": doc.get("overview", ""),
        "vibe_vector": doc.get("vibe_vector", [0.5] * 6),
        "vibe_tags": doc.get("vibe_tags", []),
        "vibe_summary": doc.get("vibe_summary", ""),
    }


@app.post("/api/vibe-rate")
def rate_vibe_movie(
    req: VibeRateRequest,
    current_user=Depends(get_current_user_optional),
    db=Depends(get_db),
):
    """Save a vibe-accuracy rating (1–5) for a movie after watching it."""
    if req.vibe_match_felt < 1 or req.vibe_match_felt > 5:
        raise HTTPException(status_code=400, detail="vibe_match_felt must be 1–5")

    user_id = current_user["_id"] if current_user else "anonymous"
    doc = {
        "user_id": user_id,
        "movie_id": req.movie_id,
        "vibe_match_felt": req.vibe_match_felt,
        "comment": req.comment,
        "timestamp": datetime.datetime.utcnow(),
    }
    db["vibe_ratings"].insert_one(doc)
    return {"message": "Vibe rating saved", "status": "ok"}
